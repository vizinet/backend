...
def create_algorithm_two_object_json(Picture, json):
    # create a new alg1 object, Default 50 radius circles
    try:
        newAlg1 = AlgorithmTwo(picture=Picture,
                               image2=ContentFile(b64decode(json['imageTwo']),
...
def edit_algorithm(request, Picture, algorithmObject, html_page):
    ...
    if request.method == 'POST':
        ...
        if hasattr(request, 'FILES'):
            form = retreive_algorithm_form(
                Picture, request.POST, request.FILES)
        ...
        if success:
            ...
            return HttpResponse(
                "Internal Server Error info: file_upload Line 182. Please contact administrator")
...
def apply_algorithm(request, picId=-1):
    ...
    if picId != -1:
        ...
        if request.method == 'POST':
            ...
            if hasattr(request, 'FILES'):
                form = retreive_algorithm_form(
                    pic, request.POST, request.FILES)
                file_data = request.FILES
            else:
            ...
            if success:
                ...
                return HttpResponse(
                    "Internal Server Error on file_upload.views in apply_algorithm")
                # TODO
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            newPic = Picture(
                ...
                algorithmType=int_to_algorithm(
                    form.cleaned_data.get('algorithmType')))
            newPic.save()
...
def upload(request):
    ...
    if request.method == 'POST':
        ...
        if toke.count() > 0:
            ...
            if 'time' in s:
                try:
                    timeTaken = datetime.datetime.strptime(
                        s['time'], "%Y.%m.%d.%H.%M.%S")
                except Exception as e:
                    print(e.message)
            ...
            try:
                ...
                algorithmList = {
                    "AlgorithmOne": create_algorithm_one_object_json(
                        newPic, s), "AlgorithmTwo": create_algorithm_two_object_json(
                        newPic, s)}
                created_algorithm_object = algorithmList[int_to_algorithm(
                    s['algorithmType'])]
            ...
            if created_algorithm_object is not None:
                try:
                    response_data['output'] = retreive_algorithm_object(
                        newPic)[0].calculatedVisualRange
                except Exception as e:
...
def view_picture(request, picId=-1, comment_num=1):
    ...
    if picId != -1:
        ...
        picture_tags = Tag.objects.filter(
            text=location).order_by("picture__uploadTime")
        pictures = []
        ...
        return render_to_response('view_image.html',
                                  {'picture': p,
                                   'computed_vr': computed_vr,
                                   'pictures': pictures,
                                   'tag': cur_tag[0],
                                      "comment_num": comment_num,
                                      'comment_form': this_comment_form,
                                   },
                                  context_instance=RequestContext(request))
